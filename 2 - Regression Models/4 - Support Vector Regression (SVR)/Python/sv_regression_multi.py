...
regressor = SVR(kernel='rbf', degree=3, epsilon=0.1)
regressor.fit(X,y)
regressor1 = SVR(kernel='linear', degree=4, epsilon=0.1)
regressor1.fit(X,y)
regressor2 = SVR(kernel='poly', degree=5, epsilon=0.1)
regressor2.fit(X,y)
regressor3 = SVR(kernel='sigmoid', degree=4, epsilon=0.1)
regressor3.fit(X,y)
...
plt.scatter(X_os, y_os, color = 'red')
plt.plot(X_os, sc_y.inverse_transform(regressor1.predict(X)), color='black')

plt.scatter(X_os, y_os, color = 'red')
plt.plot(X_os, sc_y.inverse_transform(regressor2.predict(X)), color='green')

plt.scatter(X_os, y_os, color = 'red')
plt.plot(X_os, sc_y.inverse_transform(regressor3.predict(X)), color='orange')
